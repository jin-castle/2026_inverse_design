# -*- coding: utf-8 -*-
"""Test LLM optimize + SimServer run on single record"""
import sys, tempfile
...
print(f"ID={rid} | code_len={len(code)} | full={'Yes' if rec.get('full_code') else 'No'}")

print("LLM optimize...")
fix = call_llm_optimize(rec, code)
fc = fix.get('fixed_code','')
print(f"fix_type={fix['fix_type']} | fixed_code_len={len(fc)}")
print(f"desc={fix['fix_description'][:150]}")
if fc:
    print(f"syntax OK: {check_syntax(fc)}")
    print(f"\nFirst 300 of fixed:\n{fc[:300]}")
    
    remote = f"/tmp/meep_fix_{rid}.py"
    ...
    if scp_upload(str(local_tmp), remote):
        print("Running adaptive MPI (max 300s)...")
