...
slice1d = sim.get_array(component=mp.Hz, vol=mp.Volume(center_1d, size=size_1d))

# 2D slice of Hz data
size_2d = mp.Vector3(xMax - xMin, yMax - yMin)
center_2d = mp.Vector3((xMin + xMax) / 2, (yMin + yMax) / 2)
slice2d = sim.get_array(component=mp.Hz, vol=mp.Volume(center_2d, size=size_2d))
