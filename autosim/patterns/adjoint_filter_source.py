#!/usr/bin/env python3
"""
Pattern: adjoint_filter_source
Adjoint filter source: FilteredSource, TopOptimizationProblem, design variable filtering
"""
import sys, os, time as _time
sys.path.insert(0, "/root/autosim")
from common import *  # silicon, oxide, resolution, RESULT_DIR, etc.

_PATTERN = "adjoint_filter_source"
_t0 = _time.time()
...
try:
    ...
    class FilteredSource(CustomSource):
        ...
        def cos_window_fd(self,a,f,f0):
            ...
            return cos_sum
        def sinc(self,f,f0):
            num = np.where(f==f0, self.N+1, (1-np.exp(1j*(self.N+1)*(2*np.pi)*(f-f0)*self.dt)))
