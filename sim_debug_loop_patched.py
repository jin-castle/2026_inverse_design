...
KB_API = os.environ.get("MEEP_KB_API", "http://172.19.0.2:7860")
...
def run_simulation(script_path: str, procs: int = 1, timeout: int = TIMEOUT_SEC) -> dict:
    """MPI로 시뮬레이션 실행, 결과 반환"""
    if procs > 1:
        cmd = ["mpirun", "-np", str(procs), "/home/js/miniconda/envs/pmp/bin/python", script_path]
    else:
        cmd = ["/home/js/miniconda/envs/pmp/bin/python", script_path]
