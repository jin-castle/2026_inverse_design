...
def generate_code(params: dict, paper_id: str) -> Path:
    """params.json → reproduce_{paper_id}.py 생성"""
    print(f"\n[Stage 1] 코드 생성 | design_type={params['design_type']}")
...
def run_with_error_handler(script_path: Path, executor: dict, dry_run: bool) -> Optional[dict]:
    ...
    for attempt in range(1, MAX_FIX_RETRIES + 1):
        ...
        # 실행
        stderr, stdout = _execute(script_path, env, np_, executor)

        # 결과 확인
        result_json = script_path.parent / f"{script_path.stem.replace('reproduce_','')}_results.json"
        ...
        if not stderr:
            print("  [OK] 실행 완료 (결과 파일 없음, 수동 확인 필요)")
            return {"status": "completed_no_json"}

        # 오류 분류 + 수정
        rule = classify_error(stderr, code, rules)
        if rule:
            code = apply_fix(code, rule, stderr, attempt)
            # fast-check 재검증
            tmp = script_path.parent / f"fix_attempt{attempt}_{script_path.name}"
            tmp.write_text(code, encoding="utf-8")
            if not fast_check(tmp, f"fix{attempt}"):
                print(f"  Fix #{attempt} fast-check 실패. 다음 시도...")
                continue
        else:
            print(f"  [ERROR] 매칭 규칙 없음. 수동 확인 필요.\n{stderr[-500:]}")
            _save_unknown_error(stderr, code, script_path)
            break

    print(f"  [FAIL] {MAX_FIX_RETRIES}회 시도 모두 실패")
    return None


def _execute(script_path: Path, env: str, np_: int, executor: dict):
    """실제 실행 (local Docker or SimServer)"""
    if env == "local":
        # Docker 컨테이너로 복사 + 실행
        paper_id = script_path.stem.replace("reproduce_", "")
        remote   = f"/tmp/cis_repro_{paper_id}.py"
        out_dir  = f"/tmp/cis_results_{paper_id}"

        subprocess.run(["docker", "cp", str(script_path), f"{DOCKER_CONTAINER}:{remote}"],
                       check=True, capture_output=True)
        print(f"  Docker: mpirun -np {np_} python {remote}")

        result = subprocess.run(['docker', 'exec', DOCKER_CONTAINER, "bash", "-c",
             f"mkdir -p {out_dir} && mpirun -np {np_} --allow-run-as-root "
             f"python {remote} 2>&1 | tee {out_dir}/run.log"],
            capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=int(LOCAL_TIMEOUT_H*3600+600)
        )
        # 결과 파일 가져오기
        local_out = script_path.parent
        subprocess.run(["docker", "cp", f"{DOCKER_CONTAINER}:{out_dir}/.", str(local_out)],
                       capture_output=True)
        ...
        paper_id = script_path.stem.replace("reproduce_", "")
        remote   = f"/tmp/cis_repro/{paper_id}.py"
        out_dir  = f"/tmp/cis_repro/{paper_id}_results"

        subprocess.run(["scp", "-i", SSH_KEY, str(script_path),
                        f"{SIMSERVER}:{remote}"], check=True, capture_output=True)
        print(f"  SimServer: mpirun -np {np_} python {remote}")

        # 비동기 실행
        ssh_cmd = (f"mkdir -p {out_dir} && "
                   f"nohup mpirun -np {np_} python {remote} "
                   f"> {out_dir}/run.log 2>&1 && "
                   f"echo 'DONE' >> {out_dir}/run.log")
        subprocess.run(["ssh", "-i", SSH_KEY, SIMSERVER, ssh_cmd],
                       timeout=int(executor["t_simserver_h"]*3600 + 600))

        # 결과 수집
        local_out = script_path.parent
        subprocess.run(["scp", "-r", "-i", SSH_KEY,
                        f"{SIMSERVER}:{out_dir}/.", str(local_out)],
                       capture_output=True)
...
def run_pipeline(params_path: str, dry_run=False, force_local=False, force_simserver=False):
    ...
    print(f"\n{'='*60}")
    print(f"CIS Reproduce Pipeline: {paper_id}")
    print(f"{'='*60}")

    # Stage 0: 입력 분류
    mode = classify_input(params)
    print(f"\n[Stage 0] 입력 모드: {mode} ({'코드→변환' if mode=='A' else '구조→KB적용' if mode=='B' else '역설계'})")
    ...
    if not ok:
        print("[ABORT] Fast-check 실패. 파라미터 검토 필요.")
        return

    # Stage 3: 실행 환경 결정 + 실행
    executor = decide_executor(params, force_local, force_simserver)
    result   = run_with_error_handler(script, executor, dry_run)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--force-simserver", action="store_true")
    args = ap.parse_args()

    run_pipeline(args.params, args.dry_run, args.force_local, args.force_simserver)
